...
$("#process").click(function () {
    ...
    dest = parent.image_from_web_settings.upload_path + dest
        .replace(
            "%date%",
            today.toISOString().split("T")[0]
        ).replace(      //replace date placeholder (if present) with today's date
            "%src_name%",
            file_info[0]
        ).replace(           //replace name placeholder (if present) with source name
            "%src_ext%",
            file_info[1]
        ).replace(/ /g, "-"); //replace extension placeholder (if present) with source's extension
